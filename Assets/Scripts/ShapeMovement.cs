...
public class ShapeMovement : MonoBehaviour {
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if(hitShape.transform.position.y < 0f && hitShape.transform.position.x < 2f && hitShape.transform.gameObject.layer == 11)
            {
            ...
            else if(hitShape.transform.position.y < 0f && hitShape.transform.position.x > 2f && hitShape.transform.position.x < 5f && hitShape.transform.gameObject.layer == 11)
            {
                zRotationMiddle = zRotationMiddle + 90;
                hitShape.transform.rotation = Quaternion.Euler(0f, 0f, zRotationMiddle);
            }

            else if(hitShape.transform.position.y < 0f && hitShape.transform.position.x > 5f && hitShape.transform.gameObject.layer == 11)
            {
