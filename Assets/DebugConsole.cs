...
public class DebugConsole : MonoBehaviour
{
	...
	public void InitGuis()
	{
		...
		{
			if (DebugGui == null)  // If an external Text is not set, provide the default Text
			{
		...
		else if (visible == true)
		{
			...
			else
			{
				while (x < messages.Count)
				{
					...
					{
					case "normal": gui.GetComponent<Text>().material.color = normal;
						break;
					case "warning": gui.GetComponent<Text>().material.color = warning;
						break;
					case "error": gui.GetComponent<Text>().material.color = error;
						break;
					}
